...
func TestAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_basic(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMDataFactoryLinkedServiceAzureSQLDatabaseExists(data.ResourceName),
				),
			},
...
func TestAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_update(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_data_factory_linked_service_azure_sql_database", "test")
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_update1(data),
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMDataFactoryLinkedServiceAzureSQLDatabaseExists(data.ResourceName),
...
func testCheckAzureRMDataFactoryLinkedServiceAzureSQLDatabaseExists(name string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if utils.ResponseWasNotFound(resp.Response) {
			return fmt.Errorf("Bad: Data Factory Linked Service AzureSQLDatabase %q (data factory name: %q / resource group: %q) does not exist", name, dataFactoryName, resourceGroup)
		}
...
func testCheckAzureRMDataFactoryLinkedServiceAzureSQLDatabaseDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "azurerm_data_factory_linked_service_azurerm_data_factory_linked_service_azure_sql_database" {
			continue
		}
		...
		if resp.StatusCode != http.StatusNotFound {
			return fmt.Errorf("Data Factory Linked Service AzureSQLDatabase still exists:\n%#v", resp.Properties)
		}
	}
...
func testAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_basic(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func testAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_update1(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
...
func testAccAzureRMDataFactoryLinkedServiceAzureSQLDatabase_update2(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
